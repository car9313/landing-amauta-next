...
import {AlertCircle, BarChart2, CheckCircle2, Heart, Send, Sparkles, ThumbsUp, WifiOff} from 'lucide-react';
    ...
    return (
        <section id="encuesta" className="py-12 sm:py-20 bg-white relative overflow-hidden border-t border-border/40">
            ...
            <div className="absolute top-4 right-4 z-20 flex items-center gap-2 text-xs font-bold">
                {!isOnline && (
                    <span
                        className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full flex items-center gap-1.5 shadow-sm border border-amber-200">
            <WifiOff className="w-3 h-3"/> {t('survey:offlineBadge')}
          </span>
                )}
                {isOnline && !serviceError && pendingSync && (
                    <span
                        className="bg-blue-100 text-amauta-blue px-3 py-1 rounded-full flex items-center gap-1.5 shadow-sm border border-blue-200">
            ...
            <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-amauta-blue-light/20 blur-3xl rounded-full pointer-events-none"/>
            ...
            <div className="max-w-4xl mx-auto px-5 sm:px-8 relative z-10">
                ...
                {/* Survey Box Layout */}
                <Card
                    className="bg-gradient-to-br from-slate-50 to-white rounded-3xl border border-neutral-200/80 shadow-[var(--shadow-lg)] p-4 sm:p-10 relative">
                    {/* BANNER DE SERVICIO NO DISPONIBLE (sobre la encuesta) */}
                    {serviceError && (
                        <div
                            className="mb-4 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 flex items-center gap-2 text-red-700 shadow-sm">
                            ...
                            >
                                ...
                                <div className="space-y-3">
                                    <label className="text-base sm:text-lg font-black text-amauta-blue-dark block">
                                        {t('survey:q1')} <span
                                        className="text-amauta-orange">{t('survey:required')}</span>
                                    </label>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                        {(['high', 'medium', 'low'] as const).map((interest) => (
                                            ...
                                            >
                                                <span
                                                    className="text-xl">{interest === 'high' ? '😍' : interest === 'medium' ? '🙂' : '😐'}</span>
                                                <div>
                                                    <span
                                                        className="block font-extrabold leading-none">{t(`survey:interestOptions.${interest}.label`)}</span>
                                                    <span
                                                        className="hidden sm:block text-[11px] text-foreground/40 mt-1">{t(`survey:interestOptions.${interest}.subtitle`)}</span>
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 sm:gap-3.5">
                                        {FEATURE_IDS.map((featId) => (
                                            ...
                                            >
                                                <span
                                                    className="text-2xl">{FEATURE_EMOJIS[`pref${featId.charAt(0).toUpperCase()}${featId.slice(1)}` as keyof typeof FEATURE_EMOJIS]}</span>
                                                <div>
                                                    <span
                                                        className="font-extrabold text-sm text-amauta-blue-dark block">{t(`survey:features.${featId}.title`)}</span>
                                                    <span
                                                        className="hidden sm:block text-[11.5px] font-medium text-foreground/50">{t(`survey:features.${featId}.desc`)}</span>
                                                </div>
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        <label
                                            className="text-xs font-black uppercase tracking-widest text-amauta-blue-dark block">
                                            {t('survey:q4')}
                                        </label>
                                        <Input
                                            type="text"
                        ...
                        {voted === true && (
                            ...
                            >
                                ...
                                <div className="text-center space-y-3">
                                    <div
                                        className="w-12 h-12 bg-success/15 border border-success text-success rounded-full flex items-center justify-center mx-auto shadow-sm">
                                ...
                                {/* Animated Poll Stats */}
                                <div
                                    className="bg-white border border-neutral-200/50 rounded-2xl p-5 sm:p-7 space-y-6 shadow-sm">
                                    <div
                                        className="flex items-center gap-2 pb-3 border-b border-neutral-100 font-mono text-[10px] font-black uppercase tracking-widest text-amauta-blue">
                          ...
                          <span className="text-amauta-blue-dark flex items-center gap-1.5">
                            ...
                            <span>{t('survey:statLove')}</span>
                                                    ...
                                                    <span
                                                        className="text-amauta-orange-dark font-mono font-black">{lovePct}%</span>
                                                </div>
                          ...
                          <span className="text-amauta-blue-dark flex items-center gap-1.5">
                            ...
                            <span>{t('survey:statInterest')}</span>
                                                    ...
                                                    <span
                                                        className="text-amauta-blue font-mono font-black">{interestPct}%</span>
                                                </div>
                                                <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                                                    <motion.div
                                                        initial={{width: 0}}
                          ...
                          <span className="text-amauta-blue-dark flex items-center gap-1.5">
                            ...
                            <span>{t('survey:statUnsure')}</span>
                                                    ...
                                                    <span
                                                        className="text-foreground/40 font-mono font-black">{unsurePct}%</span>
                                                </div>
                                                <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                                                    <motion.div
                                                        initial={{width: 0}}
                                            ...
                                            {sortedFeatures.length > 0 && (
                                                <div
                                                    className="pt-4 border-t border-neutral-100 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-bold text-foreground/50">
                                                    {sortedFeatures.map(({key, label, count}) => {
                                                        const pct = totalVotes > 0 ? Math.round((count / totalVotes) * 100) : 0;
                                                        return (
                                                            <div key={key}>
                                                                {FEATURE_EMOJIS[key]} {t('survey:featureHighlight', {
                                                                pct,
                                                                feature: label
                                                            })}
                                                            </div>
